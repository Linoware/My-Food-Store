...
class DateComparatorTest extends TestCase
{
    public function testConstructor()
    {
        try {
            ...
            $this->fail('__construct() throws an \InvalidArgumentException if the banners expression is not valid.');
        } catch (\Exception $e) {
            $this->assertInstanceOf('InvalidArgumentException', $e, '__construct() throws an \InvalidArgumentException if the banners expression is not valid.');
        }
    ...
    public function testTest($test, $match, $noMatch)
    {
        ...
        foreach ($match as $m) {
            $this->assertTrue($c->test($m), '->banners() tests a string against the expression');
        }

        foreach ($noMatch as $m) {
            $this->assertFalse($c->test($m), '->banners() tests a string against the expression');
        }
    }
